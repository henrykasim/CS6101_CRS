...
class recsys():
    ...
    def item_one_step_train(self, user_list, edge_index, known_pos_att_list, known_pos_att_mask, \
        ...
        # change the "_list" type of long
        user_list = user_list.to(torch.long)
        known_pos_att_list = known_pos_att_list.to(torch.long)
        known_neg_att_list = known_neg_att_list.to(torch.long)
        known_neg_item_list = known_neg_item_list.to(torch.long)
        pos_item_list = pos_item_list.to(torch.long)
        neg_item_list1 = neg_item_list1.to(torch.long)
        neg_item_list2 = neg_item_list2.to(torch.long)
